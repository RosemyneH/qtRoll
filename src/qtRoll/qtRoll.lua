if qtRollDB == nil then
  ...
  f:SetScript("OnEvent", function(self, event, rollID)
    ...
    if qtRollDB.autoNeed > 0 and isAtt and not hasAttuneProgress then
      ...
      DoRoll(1)
      return
    elseif qtRollDB.autoGreed > 0 and isBoE then
  ...
  SlashCmdList["QTROLLTEST"] = function(msg)
    ...
    local isBoE, isBoP = GetBindingTypeFromTooltip(actual)
    local isRes = RESOURCE_TYPES[itype]
    local isLock = IsLockbox(actual)
